...
#define STACKADVANCE 2.0 //Was 1.3 with SCRN, was 4.0 with CNLohr
#define STACKRANGE 15 //Was 22 with CNLohr
#define COLORMUX 0.6
#define COLORSHIFT 1.9
#define STARS_MOD 0
#define AURORA_SPEED 0.03 //was 0.06
			...
			*/
			...
			fixed4 aurora(fixed3 ro, fixed3 rd)
			{
				...
				[unroll]
				for(fixed i=0.;i<STACKRANGE;i+=STACKADVANCE)// I know, this is terrible but it's not as banding prone as lowering trinoise
				{
					fixed pt = ((.8+pow(i*1.4,1.4)*.002)-ro.y)/(rd.y*2.+0.4);
					fixed3 bpos = ro + pt * rd;
					fixed rzt = triNoise2d(bpos.zx, AURORA_SPEED);
					avgCol = (avgCol + fixed4((sin(1.5-fixed3(2.5,-.25, 1.2)+i*0.086*COLORSHIFT)*0.5+0.5)*rzt,rzt)) * .65*COLORMUX; // accumulate a little more color since we use half the steps
...
/*
	XXX NO WATER REFLECTIONS HERE!!
			...
			custom_type vertex_shader (fixed4 vertex : POSITION, fixed2 uv:TEXCOORD0, fixed3 normal : NORMAL)
			{
				...
				vs.sNormal = mul(unity_ObjectToWorld, normalize(vertex.xyz) );
				return vs;
			}
