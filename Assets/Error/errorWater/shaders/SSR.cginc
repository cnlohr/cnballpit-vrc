...
float4 reflect_ray(float4 reflectedRay, float4 rayDir, float largeRadius, float smallRadius, float stepSize, float noise, const int maxIterations)
{
	...
	float step_noise = mad(noise, 0.005, 0.025);
					...
					)
{
	...
	//largeRadius = mad(largeRadius, 0.5*noise, largeRadius);
					...
					)
{
	 ...
	 */
	/*
	int dither = floor(fmod(scrnParams.x*screenUVs.x*0.5,2)) + floor(fmod(scrnParams.y*screenUVs.y*0.5,2));
	dither *= isLowRes;
	UNITY_BRANCH if (dither == 1) {
		return float4(0,0,0,0);
	}
	*/
	/*
